...
export const SlidePreview: React.FC<SlidePreviewProps> = memo(({ slide, mode }) => {
          ...
          <div className="grid grid-cols-2 gap-4 mt-8 flex-1">
            {slide.content.map((point, idx) => (
              <div key={idx} className="p-5 bg-white/[0.03] border border-white/10 rounded-3xl flex flex-col justify-center">
                <div className="text-indigo-400 font-black text-[10px] uppercase tracking-tighter mb-2 opacity-50">Feature {idx + 1}</div>
          ...
          <div className="flex flex-col space-y-3 mt-8 flex-1 justify-center">
            {slide.content.map((point, idx) => (
              <div key={idx} className="flex items-center space-x-6 group">
                <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-indigo-500 flex items-center justify-center text-sm font-black text-white shadow-lg shadow-indigo-500/20">{idx + 1}</div>
      ...
      case 'comparison':
        return (
          <div className="grid grid-cols-2 gap-8 mt-10 flex-1">
            <div className="bg-white/[0.02] p-8 rounded-[32px] border border-white/5 relative overflow-hidden">
               <h4 className="text-indigo-400 font-black uppercase text-[10px] tracking-widest mb-6">Challenge</h4>
            ...
            <div className="bg-indigo-500/10 p-8 rounded-[32px] border border-indigo-500/20 relative overflow-hidden">
               <h4 className="text-indigo-400 font-black uppercase text-[10px] tracking-widest mb-6">Solution</h4>
               <ul className="space-y-4">
  ...
  if (mode === 'INFOGRAPHIC' && slide.imageUrl && !slide.isGeneratingImage) {
    return (
      <div className="relative w-full aspect-video rounded-[40px] overflow-hidden shadow-2xl border border-white/10 bg-slate-900">
        <img src={slide.imageUrl} className="w-full h-full object-cover" alt="Infographic Slide" />
      </div>
    );
  }
  ...
  return (
    <div className={`relative w-full aspect-video rounded-[40px] overflow-hidden shadow-2xl border border-white/5 flex flex-col ${
      ...
      {/* Visual Background for HYBRID */}
      {mode === 'HYBRID' && slide.imageUrl && !slide.isGeneratingImage && (
      ...
      {/* Loading Overlay - Simplified CSS for performance */}
      {slide.isGeneratingImage && (
        <div className="absolute inset-0 z-50 bg-slate-950 flex flex-col items-center justify-center space-y-6">
          <div className="w-16 h-16 border-4 border-indigo-500/20 border-t-indigo-500 rounded-full animate-spin"></div>
          <div className="text-center">
            <p className="text-indigo-400 font-black tracking-widest uppercase text-xs">Processing Slide Visuals</p>
          </div>
        </div>
      )}
      ...
      <div className={`relative z-10 p-14 flex flex-col h-full ${slide.layout === 'hero' ? 'justify-center items-center text-center' : ''}`}>
        <h2 className={`font-black tracking-tighter text-white transition-all ${
          ...
          <ul className={`space-y-4 mt-6 flex-1 ${slide.layout === 'hero' ? 'flex flex-col items-center justify-center' : ''}`}>
            {slide.content.map((p, i) => (
              <li key={i} className="text-xl text-slate-300 flex items-start max-w-3xl">
                <span className="text-indigo-500 mr-4 mt-2.5 w-1.5 h-1.5 rounded-full bg-indigo-500"></span>
      ...
      <div className="absolute top-10 right-10 z-20 px-4 py-2 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest text-indigo-400">
        {mode} Engine
      </div>
    </div>
  );
